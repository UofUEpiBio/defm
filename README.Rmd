...
Discrete exponential family models (DEFM) have a long tradition with a extensive
...
as Markov processes of order one, in which future states of this 0/1 vector
can be predicted as a function of the previous point in time. The following lines
of simulate the baseline data:
...
```{r echo = FALSE}
knitr::kable(head(cbind(id, Y, X)), row.names = FALSE)
```
...
dimmensions of the original dataset. In this case, the parameters used for the simulation
will be:
